...
function ResendActivationToken(props) {
	...
	const handleSubmit = (event) => {
		...
		window.axios.post( 'user/activation-token/resend', form )
			.then( response => {
				console.log(response)
				showAlert(response.data.message, 'info')
				//MessageAlert.showAlert(response.data.message, 'info')
			})
			.catch( ( error, xhr ) => {
								...
								<div className="card-body">
									<div className="p-4 rounded">
										<div className="text-center">
											<h3 className="">Resend Account Activation Token</h3>
											<p className="mb-1">Already have an account? <Link to={'/login'}>Sign in here</Link></p>
											<p>Forgot your account password? <Link to={'/forgot-password'}>Reset password here</Link></p>
										</div>
											...
											<form className="row g-3" onSubmit={handleSubmit}>
												<div className="login-separater text-center mb-4">
													...
													{alert.show && <Alert className="text-start" variant={alert.variant} onClose={() => hideAlert()} dismissible>
														<Alert.Heading as={'h5'}>Account Activation Token</Alert.Heading>
