	...
	const UserRow = (props) => {
		...
		return(
			<tr>
				...
				<td>{user.email}</td>
				<td>
					<NavLink to={"/leases/create/"+user.id} role="button" className="btn btn-primary btn-sm" title="Select user" ><i className='bx bx-user-check mx-0'/> Select</NavLink>
				</td>
			</tr>
		)
	}
			...
			<div className="card-body">
				...
				<div className="table-responsive">
					...
					{!filter && items.length===0 && <div className="col-md-7 mx-auto my-5 text-center">
						<div className="rounded rounded-5 border border-1 text-wrap p-5 fs-2">
							Start by searching for an existing tenant or create a new tenant
						</div>
					</div>}
